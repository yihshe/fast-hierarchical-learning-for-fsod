...
# TODO modify the detectron data loader back 
...
class SCGTrainer(TrainerBase):
    ...
    def __init__(self, cfg):
        ...
        # TODO LVIS this is the param for the inner loop
        # also specify the params for the outer loop
        self.start_iter = 0
        self.max_iter = cfg.SOLVER.MAX_ITER
        
        # the old hyperparams
        self.start_newton_iter = 0
        self.max_newton_iter = cfg.CG_PARAMS.NUM_NEWTON_ITER
        ...
        if isinstance(self.num_cg_iter, int):
            ...
            if self.max_newton_iter is None:
                self.max_newton_iter = 1
            self.num_cg_iter = [self.num_cg_iter]*self.max_newton_iter
        self.max_newton_iter = len(self.num_cg_iter)
        assert self.max_newton_iter!=0, "Number of Newton iteration is 0!"
    ...
    def build_hooks(self):
        ...
        if comm.is_main_process():
            ...
            ret.append(PeriodicWriterFsdet(self.build_writers(), period=1))
        return ret
    ...
    # TODO LVIS modify this function for stochastic CG
    # change the specific reference of start iter and max ter for the outer loop
    def train(self):
    ...
    def run_step(self):
        # TODO LVIS extract batches of features or a single batch of features
        start = time.perf_counter()
        proposals, box_features = self.extract_features()
        extract_time = time.perf_counter()-start
        # print('feature extracted', extract_time)
        # embed()

        # NOTE bg_class_id depends on using TFA or HDA
        # TODO LVIS or build the optimizer one time and then reset it every time (like zero grad)
        ...
        loss_dict = self.run_newton_step()
        self._write_metrics(loss_dict, extract_time)

    
    def run_newton_step(self):
        loss_dict = None
        self.newton_iter = self.start_newton_iter
        for self.newton_iter in range(self.start_newton_iter, self.max_newton_iter):
            self.cg_iter = self.num_cg_iter[self.newton_iter]
    ...
    def extract_features(self):
        ...
        # for each inner loop
        for batch_idx in range(self.cfg.SOLVER.NUM_BATCHES_PER_SET):
    ...
    # TODO change back to the default detectron2 train loader
    @classmethod
