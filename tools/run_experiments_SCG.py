...
def run_exp(cfg, configs, eval_saved_results = False):
    ...
    if eval_saved_results is False:
        ...
        if not os.path.exists(model_path):
            train_cmd = 'python -m tools.tune_net --SCGTrainer --dist-url auto --num-gpus {} ' \
...
def get_config(seed, shot):
    ...
    if args.coco:
        ...
        config_dir = 'configs/thesis/COCO-detection_{}_SCG{}'.format(model, augmentation)
        ckpt_dir = 'checkpoints/thesis/checkpoints_{}_SCG{}/coco/faster_rcnn'.format(model, augmentation)
        # TODO the path needs to be checked for seed 0
        base_cfg = '../../../Base-RCNN-FPN.yaml' if seed!=0 else '../../Base-RCNN-FPN.yaml'
    
    # NOTE SCG currently is not implemented on Pascal
    else:
        # PASCAL VOC
        ...
        config_dir = 'configs/PascalVOC-detection_{}_CG'.format(model)
        ckpt_dir = 'checkpoints_{}_CG/voc/faster_rcnn'.format(model)
        base_cfg = '../../../Base-RCNN-FPN.yaml' if seed!=0 else '../../Base-RCNN-FPN.yaml'
    ...
    configs['SOLVER']['MAX_ITER'] = ITERS[shot]
